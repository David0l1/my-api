...
router.get('/ttp', async (req, res, next) => {
  if (!req.query.type) return res.json({ status: false, error: 'Masukkan Parameter type', list: ['url', 'file'] })
  ...
  switch (req.query.type.toLowerCase()) {
    case 'file':
      await fs.readFileSync('ttp.png', buffer)
      res.sendFile(__path + '/ttp.png')
      break
    case 'url':
      res.json({ status: true, url: await upimg(buffer) })
      break
    default:
      res.json({ status: false, error: 'invalid type' })
  }
