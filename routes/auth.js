...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async(req, res) => {
    ...
    if (!email || !username || !password || !confirmpassword) {
        ...
        req.flash('error_messages',"Username Harus 4 Huruf Bang..");
        res.redirect('/signup');
    } else if(username.length > 20) {
        req.flash('error_messages',"Username Tidak Bisa >20 Karakter Lebih Bang..");
        res.redirect('/signup');
    } else if (containsEmoji(username)) {
        req.flash('error_messages',"Username Tidak Bisa Memakai Emoji Bang..");
        res.redirect('/signup');  
    }else if(!checkemail){
        req.flash('error_messages',"Ups, Kamu Pake Email Sementara.");
        res.redirect('/signup');  
    }else{
        ...
        user.findOne({ $or: [{ email: email }, { username: username }] }, function (err, data) {
            if (err) throw err;
            if (data) {
                req.flash('error_messages',"Sudah Digunakan, Coba Yang Lain");
                res.redirect('/signup');
            } else {
                bcryptjs.genSalt(12, (err, salt) => {
                    ...
                    bcryptjs.hash(password, salt, (err, hash) => {
                        ...
                        user({
                            ...
                            if (err) throw err;
                            req.flash('success_messages',"Akun Sukses Dibuat, Silahkan Masuk :D");
                            res.redirect('/login');
                        });
                    })
                });
...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified ) {
        ...
        if (check) {
        req.flash('error_messages', 'Tolong Jangan Spam, Tunggu Sekitar 30 Menit.')
        res.redirect('/docs');
        }else{
        ...
        if(mail == 'error'){
            req.flash('error_messages','Error, Coba Kembali Esok Hari');
            res.redirect('/docs');
        }else{
        req.flash('success_messages', 'Sukses Mengirim Email Verifikasi, Link Expired Setelah 30 Menit')
        res.redirect('/docs');
        }
...
router.get('/verifyemail', async (req, res) => {
    ...
    if (token) {
        ...
        if (req.user) {
            ...
            req.flash('error_messages', 'Invalid Token')
            res.redirect('/login');
        }
    }
});
