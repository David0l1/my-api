...
var creatorList = ['Onee-Dev','Davitt','Onee-Dev','Davitt','Onee-Dev','Davitt','Onee-Dev','Davitt'];
...
var tikdll = require('tiktok-scraper-without-watermark')
async function tikdl(q) {
  kaak = await tikdll.ssstik(q)
 return kaak
}
const kagApi = require('@kagchi/kag-api')
var pinte = require('../lib/pinterest.js')
var { herolist, herodetail } = require('../lib/heroml')
const fs = require('fs')
var router  = express.Router();
var axios = require('axios')
var { exec } = require('child_process')
const hit = JSON.parse(fs.readFileSync('database/apikey.json'))
...
async function addhit() {
  hit.push('caliph-api')
  fs.writeFileSync('database/apikey.json', JSON.stringify(hit))
}
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6283171710709, key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6283171710709 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
 ...
 async function getRandoms(url) {
return Math.floor(Math.random() * url)
}
 router.get('/lolivid', async (req, res, next) => {
addhit()
xirs = fs.readdirSync('./asupan')
   rendom = Math.floor(Math.random() * xirs.length)
   buffs = fs.readFileSync('asupan/'+ xirs[rendom])
   res.type('mp4')
   res.send(buffs)
   })
   router.get('/nhentaipdf', async (req, res, next) => {
     let { code } = req.query
     if (!code) return res.json({ status: 404, msg: 'Masukkan Parameter code'});
     try {
 nhdl = require('nhentaidownloader');
pdf = await nhdl(code)
  res.header("Content-Disposition", `attachment; filename="doujin.pdf"`);
res.send(pdf)
} catch (e) {
  res.json({ status: 404, msg: 'Server Error, Please Report To wa.me/6283171710709', error: String(e)})
}
   })
...
addhit()
 ...
 router.get('/wallpapercave', async (req, res, next) => {
addhit()
 if (!req.query.query) return res.json({ status: false, msg: 'Masukkan Parameter query'})
 try {
 url = (await wall.getAnimeWall2(req.query.query)).map(v => v.image)
 res.json({status: true, result: url})
 } catch {
res.json({status: false, msg: 'server Error, Please report on wa.me/6283171710709'})
...
addhit()
   yt = require('yt-search')
 if (!req.query.q) return res.json({ status: false, msg: 'Masukkan Parameter q'})
 ser = await yt(req.query.q)
 res.json({ status: 'success', result: ser.all})
 })
 router.get('/lirik', async (req, res, next) => {
addhit()
   yt = require('../lib/lirik')
 if (!req.query.q) return res.json({ status: false, msg: 'Masukkan Parameter q'})
 ser = await yt(req.query.q)
 res.json(ser)
 })
...
addhit()
 ...
 router.get('/resizeimage', async (req, res, next) => {
addhit()
if (!req.query.url) return res.send({status: `error`, res: `Masukkan param url`})
buffer = await fetch(req.query.url).then(v => v.buffer())
awa = await resize(buffer, false)
res.type(`png`)
res.send(awa)
 })
 router.get('/resizeimage/base64', async (req, res, next) => {
addhit()
if (!req.query.url) return res.send({status: `error`, res: `Masukkan param url`})
buffer = await fetch(req.query.url).then(v => v.buffer())
awa = await resize(buffer, true)
res.send({status: true, base64: awa})
 })
...
fcx = await fetch('https://raw.githubusercontent.com/Caliph91/txt/main/husbu.json')
...
afs = await fetch(`https://api.botcahx.biz.id/api/tools/ssweb?link=${req.query.url}&apikey=355ccde3`)
...
addhit()
if (!req.query.teks) return res.send({ status: 500, msg : 'Masukkan Parameter teks'})
afs = await fetch(`https://api.lolhuman.xyz/api/hartatahta?apikey=1&text=${req.query.teks}`)
ssss = await afs.buffer()
res.type('png')
res.send(ssss)
})
...
addhit()
if (!req.query.q) return res.send({ status: 500, msg : 'Masukkan Parameter q'})
upload = await pinte(req.query.q)
res.send({ status: true, url: upload })
})
async function imgbb(url) {
  data = await fetchJson(`https://api.imgbb.com/1/upload?key=761ea2d5575581057a799d14e9c78e28&image=${url}`)
  return data.data.url
}
router.get('/emoji', async (req, res) => {
const { EmojiAPI } = require('emoji-api');
const emojih = new EmojiAPI();
var { emo } = req.query
if (!emo) return res.send({ status: 404, msg: 'Masukkkan Parameter emo'})
g = await emojih.get(emo)
e = g.images
res.send({ status: 200, result: e})
})
router.get('/attp', async (req, res) => {
const { EmojiAPI } = require('emoji-api');
const emojih = new EmojiAPI();
var { text } = req.query
if (!text) return res.send({ status: 404, msg: 'Masukkkan Parameter text '})
try {
g = await fetchJson(`https://salism3api.pythonanywhere.com/text2gif/?text=${encodeURIComponent(text)}`)
buffer = await fetch(g.image).then(v => v.buffer())
res.type('webp')
res.send(buffer)
} catch {
  res.send({ status: 404, msg: 'Server Error, Please Report To wa.me/6283171710709'})
}
})
...
addhit()
  apikey = 'FreeApi'
  buffer = await fetch(`https://api.clph.me/loli?apikey=beta`).then(v => v.buffer())
  res.type('jpg')
  res.send(buffer)
...
data = await fetch(`https://api.lolhuman.xyz/api/ttp?apikey=rikkabotz&text=${encodeURIComponent(req.query.q)}`).then(v => v.buffer())
       res.type('webp')
       res.send(data)
				
         })
         router.get('/stickwm', async (req, res, next) => {
addhit()
if (!req.query.url) return res.json({ status: 404, error: 'masukkan param url'})
if (!req.query.author) return res.json({ status: 404, error: 'masukkan param author'})
if (!req.query.pack) return res.json({ status: 404, error: 'masukkan param pack'})
towebp = require('../lib/swm2')
gif2webp = require('../lib/swm2')
buff = await fetch(req.query.url).then(v => v.buffer())
base64 = await buff.toString('base64')
kk = await towebp(base64, req.query.author, req.query.pack)
res.type('webp')
res.send(kk)
})
...
router.get('/neko', async (req, res, next) => {
  addhit()
data = await fetchJson('https://api.waifu.pics/sfw/neko')
buff = await fetch(data.url).then(v => v.buffer())
res.type('png')
res.send(buff)
})
...
router.get('/y2mate', async (req, res, next) => {
  if (!req.query.url) return res.send({ status:404, msg: 'Param url tidak Boleh Kosong'})
  addhit()
  try {
buff = await fetch(req.query.url).then(v => v.buffer())
let { ext } = await require('file-type').fromBuffer(buff)
res.type(ext)
res.send(buff)
} catch (e){
  html = await fetch(req.query.url).then(v => v.text())
  res.send(html)
  console.log(e)
}
})
...
addhit()
  ...
  var { dl_link, thumb, title, filesize, filesizeF} = await yta(url5, 'id4')
	 pendek = `http://recoders-area.caliph.repl.co/api/y2mate?url=${dl_link}`
	 res.json({
	   status: true,
	   video: vid,
	  result: { 
...
addhit()
  ...
  url5 = vid.url
  server = ('id4').toLowerCase()
  var { dl_link, thumb, title, filesize, filesizeF} = await ytv(url5, 'id4')
	 pendek = `http://recoders-area.caliph.repl.co/api/y2mate?url=${dl_link}`
	 res.json({
	   status: true,
	   video: vid,
...
addhit()
 ...
 require('../lib/tiktok')(url)

         .then(async vid => {
             console.log(vid)
              res.send(vid)
         }).catch(() => res.sendFile(__path + '/routes/error.html'))
})
...
addhit()
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
    byy = await fetch("https://raw.githubusercontent.com/Caliph71/txt/main/couple.json").then(v => v.json())
...
addhit()
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
    byy = await fetch("https://raw.githubusercontent.com/Caliph91/txt/main/quotes.json").then(v => v.json())
    quotess = byy[Math.floor(Math.random() * byy.length)]
    res.json({ 
    status: true,
    quote: quotess
...
addhit()
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
          bdyyyy = await fetch('https://raw.githubusercontent.com/Caliph91/txt/main/fakta.txt')
...
router.get('/soal/math', async (req, res, next) => {
...
addhit()
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
          bdyyyy = await fetch('https://raw.githubusercontent.com/Caliph91/txt/main/darkjokes.txt')
       bdy = await bdyyyy.text()
        splitnix = bdy.split('\n')
...
router.get('/canvas/trigger', async (req, res, next) => {
  ...
  try {
    ...
    res.json({ status: 404, msg: 'Server Error, Please Report To wa.me/6283171710709'})
  }
...
addhit()
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
     if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
	 if(!text) return res.json(loghandler.nottext)
         rest = await fetch(`https://salism3api.pythonanywhere.com/write/?text=${text}`)
            ...
            request.post({
                ...
                body: `text_1=${text}&text_2=${text2}&login=OK`,
                 ...
                 fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${result}&name=${randomTextNumber}`))
                                ...
                                {
                  res.type('png');
             res.send(await fetch(data.data.url).then(v => v.buffer()))
                                })
                        })
                    }
                })
	...
	if(apikeyInput != 'FreeApi') return res.sendFile(__path + '/routes/invalidkey.html')
       ...
       kagApi.memes()
        .then(meme => {
             res.json({
                 status: true,
                 url: `https://imgur.com/${meme.hash}${meme.ext}`,
...
router.all('/*', (req, res, next) => res.send(fs.readFileSync('routes/error.html', 'utf-8')))
