...
async function cekKey(req, res, next) {
	...
	if (!apikey ) return res.json({ status : false, creator : `${creator}`, message : "! Input Apikey"})  
    ...
    if(db === null) {
		return res.json({ status : false, creator : `${creator}`, message : "! Apikey Tidak Valid"})  
				...
				return res.json({ status : false, creator : `${creator}`, message : "! Verifikasi Email Sebelum Mengunakan Apikey"})  
			} else if(db.limitApikey === 0) {
				return res.json({ status : false, creator : `${creator}`, message : "! Limit Apikey Habis"})  
			}else{
        return next();
    }
}
